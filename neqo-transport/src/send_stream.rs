...
impl SendStream {
    ...
    pub fn write_reset_frame(
        ...
        // Send RESET_STREAM at the priority of retransmissions always.
        if priority != self.priority + self.retransmission_priority {
            return Ok(false);
        };
        ...
        {
            if *frame_sent {
                return Ok(false); // Just waiting on an ACK (or a loss).
            }
            if 1 + Encoder::varint_len(self.stream_id.as_u64()) + Encoder::varint_len(err)
                > builder.remaining()
            {
                return Ok(false); // No room.
            }
            builder.encode_varint(FRAME_TYPE_RESET_STREAM);
            builder.encode_varint(self.stream_id.as_u64());
            builder.encode_varint(err);
            builder.encode_varint(final_size);
